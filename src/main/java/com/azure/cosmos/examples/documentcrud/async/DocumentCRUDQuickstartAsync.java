...
public class DocumentCRUDQuickstartAsync {
    ...
    private void createDocument() throws Exception {
        ...
        container.createItem(family,new PartitionKey(family.getLastName()),new CosmosItemRequestOptions()).block();
    ...
    private void readDocumentById() throws Exception {
        ...
        //  Read document by ID
        Family family = container.readItem(documentId,new PartitionKey(documentLastName),Family.class).block().getItem();
    ...
    private void replaceDocument() throws Exception {
        ...
        Mono<CosmosItemResponse<Family>> famResp =
                container.replaceItem(family, family.getId(), new PartitionKey(family.getLastName()), new CosmosItemRequestOptions());

        logger.info("Request charge of replace operation: {} RU", famResp.block().getRequestCharge());
    ...
    private void upsertDocument() throws Exception {
        ...
        logger.info("Request charge of upsert operation: {} RU", famResp.block().getRequestCharge());
    ...
    private void replaceDocumentWithConditionalEtagCheck() throws Exception {
        ...
        CosmosItemResponse<Family> famResp =
                container.readItem(documentId, new PartitionKey(documentLastName), Family.class).block();
        ...
        Mono<CosmosItemResponse<Family>> updatedFamResp =
                container.replaceItem(family,family.getId(),new PartitionKey(family.getLastName()),new CosmosItemRequestOptions());
        logger.info("'Concurrent' update to document " + documentId + " so ETag is now " + updatedFamResp.block().getResponseHeaders().get("etag"));
        ...
        try {
            ...
            Mono<CosmosItemResponse<Family>> failedFamResp =
                    container.replaceItem(family,family.getId(),new PartitionKey(family.getLastName()),requestOptions);

            logger.info("Request charge of replace operation: {} RU", failedFamResp.block().getRequestCharge());
    ...
    private void readDocumentOnlyIfChanged() throws Exception {
        ...
        Mono<CosmosItemResponse<Family>> failResp =
                container.readItem(documentId, new PartitionKey(documentLastName), requestOptions, Family.class);

        logger.info("Re-read doc with status code of {} (we anticipate failure due to ETag not having changed.)", failResp.block().getStatusCode());
        ...
        Mono<CosmosItemResponse<Family>> succeedResp =
                container.readItem(documentId, new PartitionKey(documentLastName), requestOptions, Family.class);
        logger.info("Re-read doc with status code of {} (we anticipate success due to ETag modification.)", succeedResp.block().getStatusCode());
